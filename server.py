...
logging.basicConfig(filename="testfile.log", format="%(asctime)s - %(levelname)s - %(message)s")
...
class WebSocket(object):
    ...
    def feed(self, data):
        ...
        if not self.handshaken:
            ...
            logger.debug("Handshake is already complete")
            ...
            # Command has to be entered
                ...
                logger.debug("Input: "+''.join(recv).strip())
                if(type(output)==str):
...
# Read the next 64 bits and interpret those as an unsigned integer (The most significant bit MUST be 0). You're done.
    ...
    def dohandshake(self, header, key=None):
        ...
        logger.debug("Begin handshake: %s" % header)
...
class WebSocketServer(object):
    ...
    def listen(self, backlog=5):
        ...
        logger.info("Listening on %s" % self.port)
        ...
        while self.running:
            ...
            for ready in rList:
                if ready == self.socket:
                    ...
                    logger.debug("Client ready for reading %s" % ready)
                    ...
                    if data:
                        ...
                        logger.debug("Closing client %s" % ready)
